<?php
  ...
  if (intval($_POST['status']) == 2) {
      ...
      $username = getValue("username", Users::uTable, "id = " . (int)$user_id);
	  ...
	  array($username, $row->title, $core->formatMoney($total->originalprice), "Completed", "MoneyBookers"), $row2->body);
      ...
      $body = str_replace(array('[USERNAME]', '[ITEMNAME]', '[PRICE]', '[STATUS]', '[PP]'), 
	  array($username, $itemname, $core->formatMoney($total->originalprice), "Failed", "MoneyBookers"), $row2->body);
